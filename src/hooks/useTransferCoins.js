...
export const useTransferCoins = ({
  ...
  const [transferCoins, { loading, error }] = useMutation(TRANSFER_COINS, {
    variables: {
      ...
      mnemonic: accountState && accountState.mnemonic
    },
    update(cache, { data }) {
      if (data) {
        const { transferCoins } = data
  ...
  return { transferCoins, loading, error }
}
